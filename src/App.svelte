...
{#if showModelDropdown}
    <div class="flex justify-center items-center top-0 ml-56">
...
<div class="ml-56">
  ...
  <div class="fixed bottom-1 right-0 left-0 z-10 flex justify-center p-2">
    <input type="text" bind:value={prompt} class="bg-gray-300 p-2 rounded-lg w-64 focus:w-96 transition-all duration-300"/>
    {#if isGenerating}
      <button onclick={() => {
  ...
  {#each responses as responses}
    ...
    {:else if responses.role === 'ollama'}
      <p class="bg-[#282828] p-2 rounded-lg text-white m-1">{@html marked(responses.ollamaContent)}</p>
    {/if}
  {/each}
